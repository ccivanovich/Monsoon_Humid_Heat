...
def calc_RH_from_T_Td(T, Td, mode=0):
    ...
    if mode == 0: 
       ...
       es = np.exp( -6096.9385 * Tk**(-1) + 21.2409642 - 2.711193e-2 * Tk +            1.673952e-5 * Tk**2.0 + 2.433502 * np.log(Tk))
       e  = np.exp( -6096.9385 * Tdk**(-1) + 21.2409642 - 2.711193e-2 * Tdk +            1.673952e-5 * Tdk**2.0 + 2.433502 * np.log(Tdk))
...
# Constants used to calculate g(T)
    ...
    d2gdT2 = 2.0*y0*rsy2rs2/(T_k*T_k*T_k) - 2.0*y0*rsy2rs2*(oty2rs)*rsdT +         y0tky1*2.0*y2*rsdT*rsdT + y0tky1*oty2rs*d2rsdT2
...
#    SHR_CONST_TKFRZ = 273.15
    ...
    if False: #ConvergenceMode:
        ...
        if any(map(len,subdo)): #len(subdo)>0:
            ...
            for www in subdo[0]:
            #    print(www)
                print('WARNING-Wet_Bulb failed to converge. Setting to T: WB, P, T, RH, Delta: %0.2f, %0.2f, %0.1f, %0.2g, %0.1f'%(Twb[www], Pressure[www],                     TemperatureK[www], relhum[www], delta[www]))
            #end
        #end
